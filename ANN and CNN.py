...
def data_augmentation(X_train, X_test, Y_train, Y_test):
    ...
    for train_index, test_index in kfold.split(X_train):
        ...
        # Fit the model on the training data
        model3.fit(generator.flow(X_train_fold, Y_train_fold, batch_size=32), epochs=2, verbose=0)
        # Extract the training, validation and testing accuracy from the evaluate method
        fold_val_loss, fold_val_acc, _, _, _ = model3.evaluate(X_val_fold, Y_val_fold, verbose=0)
        fold_train_loss, fold_train_acc, _, _, _ = model3.evaluate(X_train_fold, Y_train_fold, verbose=0)
        fold_test_loss, fold_test_acc, fold_test_pre, fold_test_rec, fold_test_auc = model3.evaluate(X_test, Y_test,
                                                                                                     verbose=0)
...
dir = '/Users/youssefsoultan/Desktop/Images/Dataset/'
...
# Normalization
X_gray = (gray_scale - gray_scale.mean()) / 255
...
# Normalization
X_rgb = (rgb_scale - rgb_scale.mean()) / 255
...
print('<==================================================================>')
print("CNN Model with Data Augmentation Results: ")
data_augmentation(X_train_rgb, X_test_rgb, Y_train_rgb, Y_test_rgb)
